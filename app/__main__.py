...
async def on_startup(
    ...
    is_replit = "replit.dev" in config.bot.DOMAIN or "localhost" in config.bot.DOMAIN
    if is_replit:
        logging.warning(
            "Replit-домен обнаружен — вебхук не изменяется, чтобы не мешать VPS."
        )
    else:
...
async def main() -> None:
    ...
    # Use MemoryStorage in Replit (no Redis service available by default).
    storage = MemoryStorage()
    # Provide a redis-compatible cache (in-process) for tasks that expect one.
    fake_redis = Redis()
